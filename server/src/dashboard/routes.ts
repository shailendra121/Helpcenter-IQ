...
import {
  ...
  getDraftByIdForAccount,
  getDraftVersionsForGap,
  listDraftsForAccount,
  saveReviewerRevision,
 ...
 *
 * Lists all generated drafts belonging to the authenticated
 * Zendesk account.
 *
 * Optional query:
  ...
  try {
    ...
    if (statusValue !== undefined) {
      if (!isReviewStatus(statusValue)) {
        return res.status(400).json({
          error:
            "status must be one of draft, in_review, approved, or rejected",
        });
      }
    ...
    const drafts = await listDraftsForAccount(
      accountId,
      status,
    );
 ...
 *
 * Returns one draft with original AI content and reviewer revision.
 */
router.get("/drafts/:id", async (rawReq, res) => {
  ...
  try {
    ...
    const draft = await getDraftByIdForAccount(
      accountId,
      draftId,
    );
 ...
 *
 * Saves the knowledge manager's edited version.
 * Original AI-generated fields are never changed.
 */
router.put("/drafts/:id", async (rawReq, res) => {
  const req = rawReq as unknown as AuthenticatedZafRequest;
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "All six draft sections must be supplied in a valid format",
      });
    }

    if (!suggested_title.trim()) {
      return res.status(400).json({
        error: "suggested_title cannot be empty",
      });
    }

    const accountId = req.zafSession.zendeskAccountId;

    await saveReviewerRevision(
      accountId,
      draftId,
      {
        suggestedTitle: suggested_title.trim(),
        problemSummary: problem_summary,
        stepByStepResolution:
          step_by_step_resolution,
        faq,
        relatedKeywords:
          related_keywords.map(
    ...
    const updated = await getDraftByIdForAccount(
      accountId,
      draftId,
    );
    ...
    return res.json({
      ...
      saved: true,
      reviewer_edited_at:
        updated?.reviewer_edited_at ?? null,
    });
  } catch (error) {
    ...
    console.error(
      "[dashboard-api] Failed to save draft revision:",
      error,
    );

    return res.status(500).json({
      error: "Failed to save reviewer revision",
    });
  }
});
 ...
 * Body examples:
 *   { "status": "in_review" }
 *   { "status": "rejected", "reason": "Missing troubleshooting steps" }
 */
router.post(
  "/drafts/:id/status",
  async (rawReq, res) => {
    ...
    try {
      ...
      return res.json({
        ...
        status_updated_at:
          updated?.status_updated_at ?? null,
      });
    } catch (error) {
      if (error instanceof DraftNotFoundError) {
        return res.status(404).json({
...
/**
 * GET /api/dashboard/drafts/:id/versions
 *
 * Returns every generated version for the draft's gap.
 */
router.get(
  "/drafts/:id/versions",
  async (rawReq, res) => {
    ...
    try {
      ...
      return res.json({
        ...
        versions: versions.map((version) => ({
          ...
          ai_title:
            version.suggested_title,
          reviewer_title:
            version.reviewer_suggested_title,
          generated_at:
            version.created_at,
        })),
      });
      ...
      console.error(
        "[dashboard-api] Failed to load draft versions:",
        error,
      );

      return res.status(500).json({
        error: "Failed to load draft version history",
      });
    }
  },
);
 ...
 *
 * Regenerates a draft using the existing HCIQ-13 generation path.
 * A new draft_articles row/version is created instead of overwriting
...
router.post(
  ...
  async (rawReq, res) => {
    ...
    try {
      ...
      const existing = await getDraftByIdForAccount(
        accountId,
        draftId,
      );
      ...
      const result = await generateDraftForGap(
        accountId,
        existing.knowledge_gap_id,
      );

      const regenerated = await getDraftByIdForAccount(
        accountId,
        result.draftId,
      );
      ...
      return res.status(201).json({
        ...
        gap_id:
          existing.knowledge_gap_id,
        version:
          regenerated?.version ?? null,
        review_status:
          regenerated?.review_status ?? "draft",
      });
    } catch (error) {
      if (
        error instanceof Error &&
